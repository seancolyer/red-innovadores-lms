(function() {
  ...
  define(['compiled/calendar/TimeBlockListManager'], function(TimeBlockListManager) {
    module("TimeBlockListManager");
    test("constructor", function() {
      var d1, d2, d3, d4, manager;
      ...
      return equal(manager.blocks[1].end, d4);
    });
    test("consolidate", function() {
      ...
      return equal(manager.blocks[2].start, d4);
    });
    test("split", function() {
      var block, d1, d2, d3, d4, d5, d6, d7, d8, expectedTimes, manager, _results;
      manager = new TimeBlockListManager();
      d1 = new Date(2011, 12, 27, 9, 0);
      d2 = new Date(2011, 12, 27, 9, 30);
      ...
      while (expectedTimes.length > 0) {
        ...
        _results.push(equal(block.end.getTime(), expectedTimes.shift().getTime()));
      }
      return _results;
    });
    test("delete", function() {
      ...
      manager["delete"](1);
      return equal(manager.blocks.length, 2);
    });
    return test("reset", function() {
      ...
      manager.add(new Date(2011, 12, 27, 8, 0), new Date(2011, 12, 27, 8, 30));
      manager.reset();
      return equal(manager.blocks.length, 0);
    });
  });
